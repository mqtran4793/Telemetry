...
async function Hyperload(sender, application_binary, progress_bar) {
  const BAUD = 576000; //
  const INITIAL_BAUD = 38400;
  app_send = sender;
  hyperload_activated = true;

  serial_controller.baud(INITIAL_BAUD);
  serial_controller.pause(false);
  await sleep(10);
  ...
  while (breaker) {
    ...
    switch (state) {
      ...
      case HyperloadStates.FindHyperload:
        ...
        let hyperload_signature = await serial_controller.readByte(5000);
      ...
      case HyperloadStates.SetBaudRates:
        ...
        if (acknowledge_byte === control_word[0]) {
          ...
          console.error("Failed to receive Control Word Ack expected = ",
            control_word[0], "recieved = ", acknowledge_byte);
          state = HyperloadStates.BailOut;
        }
        break;
      ...
      case HyperloadStates.TransmitApplicationToBoard:
        ...
        if (current_block != total_blocks) {
          ...
          console.debug("Final block flashed, Flashing Successful!");
          success = true;
        }
        state = HyperloadStates.BailOut;
        break;
